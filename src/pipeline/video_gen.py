...
class VideoGenerator:
    ...
    def _init_fal_client(self) -> None:
        ...
        if self.dry_run or self.simulate:
            logger.info("Running in test mode - skipping fal client initialization")
            return
    ...
    def _build_video_prompt(self, scene: Scene, image_path: Optional[str] = None) -> str:
        """
        ...
        logger.info(f"Generated prompt for scene {scene.id}: {prompt[:100]}...")
        return prompt
    ...
    def _check_budget(self, estimated_cost: float) -> None:
        ...
        if total_cost > self.budget_limit:
            raise BudgetExceededException(
                f"Operation would exceed budget: ${total_cost:.2f} > ${self.budget_limit:.2f}"
            )
    ...
    def _call_fal_api(self, prompt: str, image_path: Optional[str] = None) -> Optional[Dict]:
        """
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                logger.info(f"Calling fal.ai API (attempt {attempt + 1}/{MAX_RETRIES + 1})")
                ...
                logger.error(f"fal.ai API call failed (attempt {attempt + 1}): {e}")
    ...
    def generate_scene_video(self, 
        ...
        try:
            ...
            logger.error(f"Unexpected error generating video for scene {scene.id}: {e}")
            return None
